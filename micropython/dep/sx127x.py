...
class SX127x:
    ...
    def __init__(self,
                 name = 'SX127x',
                 parameters = {'frequency': 868.3E6, 'tx_power_level': 2, 'signal_bandwidth': 125E3,
                               'spreading_factor': 7, 'coding_rate': 1, 'preamble_length': 12,
    ...
    def init(self, parameters = None):
        ...
        if version != 0x12:
            ...
            version = self.readRegister(REG_VERSION)
            print("Found version %s" % str(version))
            if version != 0x12:
                print("Invalid version %s" % str(version))
